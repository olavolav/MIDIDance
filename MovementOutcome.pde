    ...
    for (int time_lag=0; time_lag<LENGTH_OF_PAST_VALUES_FOR_BAYESIAN_ANALYSIS; time_lag++) {
      for (int axis_index=0; axis_index<NUMBER_OF_SIGNALS; axis_index++) {
        if(input.axis_dim[axis_index].signal_group == this.associated_signal_group) {
          // Compute the posterior probability according to indipendent normal distributions
          // for each involved axis (i.e. each axis of the associated signal group).
          log_probability += log( 1.0/(std_target_move[axis_index][time_lag]*sqrt(2.0*PI)) * exp( -0.5*pow( (input.axis_dim[axis_index].last_values_buffer[time_lag] - avg_target_move[axis_index][time_lag]) / std_target_move[axis_index][time_lag], 2) ) );
        }
      }
    }
